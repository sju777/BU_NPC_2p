...
% Based on script_MTH_Stim_Main.m.
% Adds a "Configure stimulus block" section (below, after trial timing)
% to set the number of trials in a block and the percentage of trials
% assigned to each stimulus type: audio-only, airpuff(whisker)-only, and
% paired. The main stimulus-generation structure is unchanged - each
% trial still plays the same single-trial waveform built below; for
% audio-only trials the airpuff channel is simply set to 0V for that
% trial, and for airpuff-only trials the audio channel is set to 0V
% (paired trials keep both channels at their configured amplitude).
...
%\|/
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% trial.N is set in the "Configure stimulus block" section below.
...
%% Configure stimulus block: total trials & trial-type percentages
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% EDIT THESE VALUES to set the block size and the percentage of trials
% assigned to each stimulus type. Percentages must sum to 100.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
trial.N    = 30; % total number of trials in the block
pctAirpuff = 10; % % of trials: airpuff-only (whisker stimulus; audio set to 0V)
pctAudio   = 10; % % of trials: audio-only (airpuff set to 0V)
pctPaired  = 80; % % of trials: paired (both at full amplitude)

if abs((pctAirpuff+pctAudio+pctPaired)-100)>eps
    error('pctAirpuff + pctAudio + pctPaired must sum to 100 (currently %g).',pctAirpuff+pctAudio+pctPaired)
end

% Trial counts per type, rounded to the nearest whole trial. 'paired'
% absorbs the rounding remainder so the counts always sum to trial.N.
nAirpuff=round(trial.N*pctAirpuff/100);
nAudio=round(trial.N*pctAudio/100);
nPaired=trial.N-nAirpuff-nAudio;

trial.labels=[repmat({'airpuff'},1,nAirpuff),repmat({'audio'},1,nAudio),repmat({'paired'},1,nPaired)];
trial.labels=trial.labels(randperm(trial.N)); % shuffle trial order

% OPTIONAL: for an exact, hand-picked sequence instead of the random mix
% above, overwrite trial.labels directly (must have trial.N entries, each
% one of 'airpuff'/'audio'/'paired'), e.g.:
% trial.labels = {'airpuff','audio','paired','paired','paired', ...};

fprintf('\nTrial type sequence (%d trials: %d airpuff, %d audio, %d paired)', trial.N, nAirpuff, nAudio, nPaired)
clearvars nAirpuff nAudio nPaired pctAirpuff pctAudio pctPaired
...
run.VTrial=zeros(device.outputRate*trial.ISI,size(device.outputChannel,2)); % one trial, both stimuli at full amplitude
...
%% Assemble the block's full output waveform (zero out unused channel per trial)
% Each trial reuses the single-trial waveform (run.VTrial) built above.
% For 'audio' trials the airpuff channel is set to 0V for that trial; for
% 'airpuff' trials the audio channel is set to 0V; 'paired' trials keep
% both channels as defined above. The full run waveform (run.VOut)
% concatenates one ISI-length block per trial, in the order given by
% 'trial.labels' (set in the "Configure stimulus block" section above).
audioChannelInd=find(strcmp({device.outputChannel.name},'audio'));
airpuffChannelInd=find(strcmp({device.outputChannel.name},'airpuff'));

run.VOut=zeros(device.outputRate*trial.ISI*trial.N,size(device.outputChannel,2));
for iTrial=1:trial.N
    tmpVTrial=run.VTrial;
    switch trial.labels{iTrial}
        case 'audio'
            tmpVTrial(:,airpuffChannelInd)=0; % 0V airpuff during audio-only trials
        case 'airpuff'
            tmpVTrial(:,audioChannelInd)=0;   % 0V audio during airpuff(whisker)-only trials
        case 'paired'
            % keep both channels at their configured amplitude
        otherwise
            error('Unknown trial type: %s', trial.labels{iTrial})
    end
    tmpRows=(iTrial-1)*size(run.VTrial,1)+(1:size(run.VTrial,1));
    run.VOut(tmpRows,:)=tmpVTrial;
end
clearvars tmp*
...
fprintf('\nTrials...')
% run.VOut already contains the full block sequence (see 'Assemble the
% block's full output waveform...' above), so it is played once, not
% repeated.
