...
% No GUI - instead, see "Configure stimulus block" below to set the
% number of trials in a block and the percentage of trials assigned to
% each stimulus type (airpuff/whisker-only, audio-only, paired). Trials
% are randomly shuffled into that mix; edit trial.labels directly there
% if you want an exact hand-picked sequence instead.
...
% are delivered on that trial. 'airpuff' is the whisker stimulus. The
% 'paired' type activates both 'airpuff' and 'audio' together.
...
pctAirpuff = 10; % % of trials: airpuff-only (whisker stimulus)
pctAudio   = 10; % % of trials: audio-only
pctPaired  = 80; % % of trials: paired (airpuff+audio)
...
% channel stays at 0V for that trial. The full run waveform (run.VOut)
% concatenates one ISI-length block per trial, in the order given by
% 'trial.order' (set in the "Configure stimulus block" section above), so
% airpuff-only, audio-only, and paired trials are interleaved within the
% block.
